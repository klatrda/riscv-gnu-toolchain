/* GCC option-handling definitions for the Pulp architecture
   Support for .Pulp_Chip.Info section. Done here in a centralized way  for gcc and binutils.
...

{
	static unsigned long long int Pulp_Compatible_Processors[PULP_LAST] = {
		...
		/* PULP_SLIM_DEV */	((1<<PULP_RISCV) | (1<<PULP_SLIM) | (1<<PULP_SLIM_DEV)),
...
static enum Pulp_Processor_Type PulpDecodeCpu(const char *Text, int *Len)

{
	...
	for (i = 0; Dup[i]; i++) Dup[i] = TOLOWER (Dup[i]);
        ...
        else if (strncmp (Dup, "pulpslimdev", 11) == 0)   { free(Dup); if (Len) *Len = 11; return PULP_SLIM_DEV; }
...

{
        switch (Which) {
                ...
                case PULP_SLIM_DEV: return "pulpslimdev";
                default: return "Error";
        }
}
